...
<form id="" method="POST">
    ...
    <div class="form-group">
            <label>Select Volunteer ID:</label>
            <select name="Flight_route_route_id" required class="form-control">
                <option selected></option>
                <?php
                $result = mysqli_query($con,"SELECT * FROM fingerprint_details");
                while($row = mysqli_fetch_array($result))
                {
                    echo '<option value="'.$row['Fingerprint_Volunteer_Id'].'">';
                    echo $row['Fingerprint_Volunteer_Id'];
                    echo '</option>';
                }
                ?>
            </select>
    ...
    <div class="form-group label-floating">
        <label class="control-label">Figerprint Id &nbsp; <a href="javascript:window.open('all-prints.php','mypopuptitle','width=700,height=400')"></label>
